...
//			direction:			"ltr",
			...
			buttons: 			"bold,italic,underline,strike,sup,sub,|,img,video,link,|,bullist,numlist,|,fontcolor,fontsize,fontfamily,|,justifyleft,justifycenter,justifyright,|,quote,code,table,removeFormat,chart",
			allButtons: {
				chart: {
					title: CURLANG.chart,
					buttonHTML: '<i class="fa fa-area-chart"></i>',
					excmd: "chart",
					modal: {
						title: CURLANG.video,
						width: "600px",
				...
				if (r.html) {
					...
					$.each(r.input,$.proxy(function(j,inp) {
						...
						if (inp.type && inp.type=="div") {
							...
							$c.append(this.strf('<div class="wbbm-inp-row">' +
							'<label>{title}</label>' +
							'<div class="inp-text div-modal-text" contenteditable="true" name="{param}">{value}</div>' +
							'</div>'
							,inp));
						}else{
							//default input
							...
							'<label>{title}</label>' +
							'<input class="inp-text modal-text" type="text" name="{param}" value="{value}"/>' + 
